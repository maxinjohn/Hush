...
Hush is a personal fork of [ArchiveTune](https://github.com/ArchiveTuneApp/ArchiveTune). This notice covers the Android Hush app in this repository (`app.hush.music`). It explains what the app stores on your device, what it can send to external services when you use specific features, and what Android permissions it requests.

This notice is based on the current source code and build configuration. It does not replace the privacy terms of YouTube or YouTube Music, Last.fm, ListenBrainz, GitHub, lyrics providers, the ArchiveTune canvas service, or any Together server you choose to use.

## Privacy Summary

- Most core app data is stored locally on your device.
- Hush does not secretly harvest, sell, or broker your personal data.
- Hush does not silently send your data to unrelated third-party services.
...
Hush does not silently forward your data to unrelated services. It only contacts external services when you use online features, and the exact payload depends on the feature you use and how you configure it.
...
| Local database contents | The Room schema includes songs, artists, albums, playlists, search history, lyrics, audio format metadata, and playback event records. | `app/schemas/app.hush.music.db.InternalDatabase/9.json` |
| Settings and tokens stored locally | DataStore preference keys include UI settings, proxy settings, history toggles, Together values, YouTube session values, account name or email fields, Last.fm session values, ListenBrainz token values, and update-cache keys. | `app/src/main/kotlin/moe/archivetuneapp/archivetune/constants/PreferenceKeys.kt` |
...
| Canvas service requests | The canvas module sends song and artist names, album IDs, or album URLs to `https://artwork-archivetune.koiiverse.cloud/` and can attach a bearer token. | `canvas/src/main/kotlin/moe/archivetuneapp/archivetune/canvas/HushCanvas.kt` |
