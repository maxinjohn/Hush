...
import moe.rukamori.archivetune.constants.AlarmEnabledKey
import moe.rukamori.archivetune.constants.AlarmEntriesKey
import moe.rukamori.archivetune.constants.AlarmHourKey
import moe.rukamori.archivetune.constants.AlarmMinuteKey
import moe.rukamori.archivetune.constants.AlarmNextTriggerAtKey
import moe.rukamori.archivetune.constants.AlarmPlaylistIdKey
import moe.rukamori.archivetune.constants.AlarmRandomSongKey
...
object MusicAlarmStore {
    ...
    suspend fun save(
        context: Context,
        entries: List<MusicAlarmEntry>,
    ) {
        saveProtected(context, entries)
        context.dataStore.edit { prefs ->
            prefs[AlarmEntriesKey] = serialize(entries)
            prefs[AlarmNextTriggerAtKey] =
                entries
                    .filter { it.enabled }
                    .minOfOrNull { it.nextTriggerAt.takeIf { time -> time > 0L } ?: Long.MAX_VALUE }
