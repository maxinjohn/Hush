...
constructor(
        ...
        scope.future(Dispatchers.IO) {
            val items =
                when (parentId) {
                    ...
                    MusicService.HOME_FORGOTTEN_FAVORITES -> database.forgottenFavorites().first()
                        .shuffled()
                        .take(AUTO_BROWSE_LIMIT)
                        .map { it.toMediaItem(parentId) }

                    MusicService.HOME_KEEP_LISTENING -> homeKeepListeningSongs()
                        .map { it.toMediaItem(parentId) }

                    MusicService.HOME_SUGGESTED_SONGS -> homeSuggestedSongs()
                        .map { it.toMediaItem(parentId) }

                    MusicService.HOME_MIXES_AND_RADIOS -> homeMixesAndRadios()
    ...
    override fun onSetMediaItems(
        ...
        scope.future(Dispatchers.IO) {
            ...
            when (path.firstOrNull()) {
                MusicService.HOME_QUICK_PICKS -> {
                    val songs = database.quickPicks().first().shuffled().take(AUTO_BROWSE_LIMIT)
                    songs.toMediaItemsWithStartPosition(path.getOrNull(1), startPositionMs)
                }

                MusicService.HOME_FORGOTTEN_FAVORITES -> {
                    val songs = database.forgottenFavorites().first().shuffled().take(AUTO_BROWSE_LIMIT)
                    songs.toMediaItemsWithStartPosition(path.getOrNull(1), startPositionMs)
                }
                ...
                MusicService.ONLINE_PLAYLIST -> {
                    ...
                    val songs = onlinePlaylistSongs(playlistId).let { songs ->
                        if (action == PLAYLIST_ACTION_SHUFFLE) songs.shuffled() else songs
                    }
                    if (action == PLAYLIST_ACTION_SHUFFLE) {
                        withContext(Dispatchers.Main.immediate) {
        ...
        return when (path.getOrNull(2)) {
            null -> {
                val actionItems = buildList {
                    ...
                    val currentSongId =
                        withContext(Dispatchers.Main.immediate) {
                            session.player.currentMediaItem?.mediaId?.trim()
            ...
            PLAYLIST_ACTION_SORT ->
                if (path.size == 3) {
                    ...
                    val sortOption = playlistSortOption(path.getOrNull(3), path.getOrNull(4))
                        ?: return emptyList()
    ...
    private suspend fun playlistItem(mediaId: String): MediaItem? {
        ...
        return when (path.getOrNull(2)) {
            null -> playlistHeaderItem(playlistId)
            PLAYLIST_ACTION_SHUFFLE ->
                if (path.size == 3) {
                    queueMediaItem(
                        mediaId,
    ...
    private fun sortDownloadedSongs(
        ...
        val sorted =
            when (option.sortType) {
                ...
                PlaylistSongSortType.NAME -> songs.sortedWith(songTitleComparator())
                PlaylistSongSortType.ARTIST -> songs.sortedWith(songArtistComparator())
    ...
    private fun List<PlaylistSong>.sortedPlaylistSongs(
        sortOption: AutoPlaylistSortOption?,
    ): List<PlaylistSong> {
        ...
        val sorted =
            when (option.sortType) {
                ...
                PlaylistSongSortType.ARTIST ->
                    sortedWith(compareBy(songTitleCollator()) { playlistSong ->
    ...
    private fun playlistSortOption(
        ...
        val parsedSortType = sortType?.let {
    ...
    private fun PlaylistSongSortType.toSongSortType(): SongSortType =
        when (this) {
            PlaylistSongSortType.CUSTOM,
            PlaylistSongSortType.CREATE_DATE -> SongSortType.CREATE_DATE
    ...
    private fun songTitleComparator(): Comparator<Song> =
        compareBy(songTitleCollator()) { it.song.title }
