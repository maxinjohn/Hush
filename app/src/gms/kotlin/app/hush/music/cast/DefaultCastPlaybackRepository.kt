...
private class GmsCastMediaItemConverter(
    ...
    private fun MediaItem.resolvePlayableMediaId(fallback: MediaQueueItem): String? {
        ...
        (localConfiguration?.tag as? app.hush.music.models.MediaMetadata)
            ?.id
            ?.trim()
            ?.takeIf { it.isNotEmpty() }
            ?.let { return it }
        ...
        return app.hush.music.models.MediaMetadata(
            ...
            artists =
                artistText
                    ...
                    ?.map { app.hush.music.models.MediaMetadata.Artist(id = null, name = it) }
                    .orEmpty(),
            duration = -1,
            thumbnailUrl = castMetadata?.images?.firstOrNull()?.url?.toString(),
            album =
                albumTitle?.let {
                    app.hush.music.models.MediaMetadata.Album(
                        id = it,
                        title = it,
                    )
                },
