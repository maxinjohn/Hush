...
### Waze Bridge
- **Queue always correct** — Waze media panel queue now survives MusicService restarts and playlist switches. Shim uses content fingerprinting (not just revision numbers) to detect queue changes, so the queue is always up to date even when the app restarts. Monotonic revision counter persisted across service restarts. *(Hush)*
- **Queue display** — Waze now shows the full queue with track names, artists, and artwork via a two-level MediaBrowser hierarchy (root → "Hush Queue" → tracks). *(Hush)*
- **Auto-play on tap** — Tapping a song in Waze's queue starts playback automatically (skip + play command). *(Hush)*
- **Play/pause fix** — broadcast fallback now reaches MusicService's WazeCommandReceiver even when startForegroundService fails on Android 14+. *(Hush)*
- **Previous track button** — shim notification shows Prev/Pause/Play/Next. *(Hush)*
- **CoroutineScope leak fixed** — replaced unmanaged CoroutineScope with managed serviceScope cancelled on cleanup. *(Hush)*
- **Debounce map cleanup** — lastCommandTimeByType prunes stale entries every 50 commands to prevent memory growth. *(Hush)*

### Search & Playback
- **URL paste-to-play** — paste a YouTube/YT Music link into the search field and it plays the video, opens the album/playlist, or navigates to the artist directly. Supports watch URLs, youtu.be short links, playlist links, and channel links. *(Hush)*
- **Crossfade timing fix** — crossfade now works correctly at non-default playback speeds (e.g. 1.5x, 2x). Blend duration scales with speed to maintain consistent wall-clock fade. *(Hush)*

### YouTube Comments
- **Comments section per song** — view YouTube comments for any playing song via Player Menu → Comments. Full comment list with author avatars, timestamps, like counts, and heart indicators. Infinite scroll for pagination. *(Hush)*

### Existing Features (inherited from ArchiveTune 14.x base)
- Self-healing remote player config (CipherConfigFetcher) — automatically disables blocked YouTube clients and updates cipher patterns via remote config
- Blocked artists filtering across queues, playlists, and liked songs
- Hide music videos toggle
- Equalizer with AutoEq profile import
- Musixmatch lyrics provider with priority selection
- Audio output switcher (phone/BT/cast picker)
- Background music recognition via Quick Settings tile
- Always-On Display
- BetterLyrics Portato word-synced provider
- Spotify playlist import/download
- M3U/CSV import with review-before-save
- Scheduled backups with retention
- Infinite queue bootstrapping
- Play Next via right-swipe
- Apple Music/Tidal canvas backgrounds
