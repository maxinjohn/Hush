...
package app.hush.music.moriextractor
    ...
    suspend fun extractAudioUrl(
        ...
        withContext(Dispatchers.IO) {
            ...
            if (normalizedVideoUrl.isBlank()) {
                throw HushExtractorException("Video URL is missing")
            }
            ...
            if (token.isBlank()) {
                throw HushExtractorException("Hush Extractor token is missing")
            }
            ...
            try {
                ...
                val response =
                    try {
                        ...
                        throw HushExtractorException(
                            "Hush Extractor returned invalid JSON",
                            serialization,
                        )
                    }
                    ...
                    val message =
                        response.error
                            ...
                            ?: if (response.success && !response.audioUrl.isNullOrBlank()) {
                                "Hush Extractor returned raw audio_url but no stream_url. Upload/run the stream proxy main.py on the backend."
                            } else {
                                "Hush Extractor returned no playable proxy stream"
                            }
                    _extractorState.value = ExtractorState.Error(message)
                    throw HushExtractorException(message)
                }
                ...
                val exception =
                    throwable as? HushExtractorException
                        ?: HushExtractorException("Hush Extractor request failed", throwable)
